...
$card=$row_Recsn['card'];
$fus=$_SESSION['gfus'];$newnr=$_SESSION['newnr'];$pudid=$_SESSION['pudid'];$vfdcard=$_SESSION['fdcard'];//echo $newnr,"##",$vfdcard;exit;
?>
...
$c_num2=$row_Reccd['number'];
$fname2=$row_Reccd['name'];
...
if ($row_Recapud['fdm'] > 1) {
    ...
    $Result11 = mysql_query($update11, $sc) or die(mysql_error());
      /*  //我系大公排
	    ...
	    mysql_select_db($database_sc, $sc);
        $query_Recgg = sprintf("SELECT * FROM fd WHERE c_guser='$vfdcard'");
	...
	if ($totalRows_Recgg == 0) {$gw="L";} else {if ($row_Recgg['gtow'] == "L") {$gw="R";} else {$gw="L";}}
		...
		if ($totalRows_Recxf == 0) {
			mysql_select_db($database_sc, $sc);
			$insertCommand13="INSERT INTO fd (name, number, card, c_fuser, c_guser, gtow, year, moom, day, end_y, end_m, end_d, date, time) VALUES ('$fname', '$c_num', '$fdcard', '$vfdcard', '$vfdcard', '$gw', '$year', '$moom', '$day', '$nyear', '$moom', '$day', '$date', '$time')"; 
			mysql_query($insertCommand13,$sc);
		}
		/*
		mysql_select_db($database_sc, $sc);
