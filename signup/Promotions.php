		...
		function purchase_event($number){
			...
			if($top_c_cash >= $my_p && $top_st == 1 && $top_assessment == 1)
			{
				...
				if ($totalRows_Recxf == 0) {
					...
					$fd_amount = $my_p;
					$insertCommand13="INSERT INTO fd (number, card, name, c_fuser, c_guser, gtow, filling_position, fd_amount, year, moom, day, end_y, end_m, end_d, at, date, time, note) VALUES ('$number', '$fdcard', '$m_nick', '$fcard', '$g_user', '$gw', '$index','$fd_amount', '$year', '$moom', '$day', '$nyear', '$moom', '$day', 0, '$date', '$time', '自動購球')"; 
