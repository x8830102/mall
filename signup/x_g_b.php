...
if (($totalRows_Reccd31-1) > $totalRows_Reccd3) {$keyok=1;} else {
	mysql_select_db($database_sc, $sc);
...
mysql_query($insertCommand13,$sc);
    ...
    if ($totalRows_Recs == 0) {
        ...
        mysql_query($insertCommand15,$sc);
	        ...
	        $update11="UPDATE gold_sum SET level2= $new_level1 WHERE number = '$c_num' && year=$year && moom=$moom && day=$day";
            mysql_select_db($database_sc, $sc);
            $Result11 = mysql_query($update11, $sc) or die(mysql_error());
	        }
		//
