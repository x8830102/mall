...
$card=$row_Recsn['card'];
$fus=$_SESSION['gfus'];$newnr=$_SESSION['newnr'];$pudid=$_SESSION['pudid'];$vfdcard=$_SESSION['fdcard'];//echo "##e-",$vfdcard;exit;
?>
...
<table width="400" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td align="center">進度： 8 / 8</td>

  </tr>

</table>
<script type="text/javascript">document.location.href="http://lifelink.cc/Cup_shunt.php?note=<? echo $_SESSION['notes']?>";</script>
<?php exit;?>
<?php
/******************************
再檢查一次出局?
*******************************/
$ope_total=0;
mysql_select_db($database_sc, $sc);$atm=0;
$query_Reccd = sprintf("SELECT * FROM fd WHERE at=$atm ORDER BY id");
$Reccd = mysql_query($query_Reccd, $sc) or die(mysql_error());
$row_Reccd = mysql_fetch_assoc($Reccd);
$totalRows_Reccd = mysql_num_rows($Reccd);//echo $totalRows_Reccd,"!!!<br/>";
if ($totalRows_Reccd != 0) {
	do {$c_num=$row_Reccd['number'];
        $fname=$row_Reccd['name'];
        $tt_us=$row_Reccd['card'];//echo $tt_us,"#",$c_num;//exit;
		$tid=$row_Reccd['id'];
		$fdsql="fd";$keyok=0;
		//if ($c_num == "SN1611110064") {echo $tt_us;exit;}
//
mysql_select_db($database_sc, $sc);
$query_Recm2 = sprintf("SELECT * FROM memberdata WHERE number = '$c_num' && m_ok=1");
$Recm2 = mysql_query($query_Recm2, $sc) or die(mysql_error());
$row_Recm2 = mysql_fetch_assoc($Recm2);
$totalRows_Recm2 = mysql_num_rows($Recm2);
$a_pud=$row_Recm2['a_pud'];//echo "-apud=",$a_pud;
mysql_select_db($database_sc, $sc);
	...
	$apud_a=$row_Recapud['p'];//echo "-p=",$apud_a;
...
$fdsql="fd";$pcpin=20000;$pcpout=40000;
//7p-yes
$p_total=0;$gusa="";
mysql_select_db($database_sc, $sc);
...
$totalRows_Recg = mysql_num_rows($Recg);//echo "==",$totalRows_Recg;
if ($totalRows_Recg != 0) {$p_total=$p_total+$totalRows_Recg;
    ...
    $totalRows_Recg2 = mysql_num_rows($Recg2);//echo "==",$totalRows_Recg2;
	if ($totalRows_Recg2 != 0) {$p_total=$p_total+$totalRows_Recg2;}
	} while ($row_Recg = mysql_fetch_assoc($Recg));
	}//echo $p_total,"##",$tt_us;exit;
//echo "@",$p_total,"###";//,$tt_us;//,"<br/>"
		//pf

if ($p_total == 6) {
	if ($c_num != "gugold777") {
	$ope_total++;echo $c_num,"@",$tt_us,"#";
    	mysql_select_db($database_sc, $sc);
        $query_Reccd2 = sprintf("SELECT * FROM $fdsql WHERE number = '$c_num' ORDER BY id");
		...
		$cid=$row_Reccd2['id'];
		$c_name=$row_Reccd2['number'];
		$c_cnum=$row_Reccd2['card'];//echo $cid;exit;
/*
//allsum 
mysql_select_db($database_sc, $sc);
$query_Recma = sprintf("SELECT * FROM memberdata WHERE number='$c_name'");
$Recma = mysql_query($query_Recma, $sc) or die(mysql_error());
$row_Recma = mysql_fetch_assoc($Recma);
$totalRows_Recma = mysql_num_rows($Recma);
$new_allsum=$row_Recma['allsum']+300000;
if ($new_allsum > 400000) {$new_allsum=400000;}
$update11="UPDATE memberdata SET allsum=$new_allsum WHERE number = '$c_name'";
mysql_select_db($database_sc, $sc);
...
mysql_select_db($database_sc, $sc);
$query_Reccd31 = sprintf("SELECT * FROM $fdsql WHERE c_fuser = '$c_cnum'");
...
//if ($keyok == 1) {echo $tt_us,"$$<br/>";exit;}
////出局
if ($p_total == 6 && $keyok == 1) {//echo "出局";exit;
    if ($c_num == "gugold777") {
	//p-o
		$apud_o=10000;
	mysql_select_db($database_sc, $sc);
    $query_Recpo = sprintf("SELECT * FROM pay_o ORDER BY id DESC");
    $Recpo = mysql_query($query_Recpo, $sc) or die(mysql_error());
    $row_Recpo = mysql_fetch_assoc($Recpo);
    $totalRows_Recpo = mysql_num_rows($Recpo);
    $newpsum=$row_Recpo['psum']+$apud_o;$pat=1;
    mysql_select_db($database_sc, $sc);
    $insertCommand11="INSERT INTO pay_o (pin, psum, number, card, at, date, time, pud_name) VALUES ('$apud_o', '$newpsum', '$c_num', '$tt_us', '$pat', '$date', '$time', '$pud_name')"; 
    mysql_query($insertCommand11,$sc);
	    //p-p
		$apud_p=10000;
	mysql_select_db($database_sc, $sc);
    $query_Recpp = sprintf("SELECT * FROM pay_p ORDER BY id DESC");
    $Recpp = mysql_query($query_Recpp, $sc) or die(mysql_error());
    $row_Recpp = mysql_fetch_assoc($Recpp);
    $totalRows_Recpp = mysql_num_rows($Recpp);
    $newpsum=$row_Recpp['psum']+$apud_p;$pat=1;
    mysql_select_db($database_sc, $sc);
    $insertCommand11="INSERT INTO pay_p (pin, psum, number, card, at, date, time, pud_name) VALUES ('$apud_p', '$newpsum', '$c_num', '$tt_us', '$pat', '$date', '$time', '$pud_name')"; 
    mysql_query($insertCommand11,$sc);
	    }
		//fd-number
	...
	if ($totalRows_Recgg == 0) {$gw="L";} else {if ($row_Recgg['gtow'] == "L") {$gw="R";} else {$gw="L";}}
		...
		mysql_select_db($database_sc, $sc);
$insertCommand13="INSERT INTO fd (name, number, card, c_fuser, c_guser, gtow, year, moom, day, end_y, end_m, end_d, date, time) VALUES ('$fname', '$c_num', '$fdcard', '$c_cnum', '$xgu', '$gw', '$year', '$moom', '$day', '$nyear', '$moom', '$day', '$date', '$time')"; 
mysql_query($insertCommand13,$sc);}
        $vf=$fdcard;
		//out
mysql_select_db($database_sc, $sc);
...
$newcpsum=$row_Recpc['psum']-$pcpout;$pat=1;
mysql_select_db($database_sc, $sc);
$insertCommand13="INSERT INTO pay_b (pout, psum, number, card, at, date, time, year, moom, day) VALUES ('$pcpout', '$newcpsum', '$c_num', '$tt_us', '$pat', '$date', '$time', '$year', '$moom', '$day')"; 
mysql_query($insertCommand13,$sc);
        //
		$level=2;$gold_f=$pcpout;
		$fnote="開福卡積分(".$fd_t_card.")";
		$sncode=$c_num.date("ymdhis");
	    mysql_select_db($database_sc, $sc);
        $insertCommand15="INSERT INTO gold_m (number, year, moom, day, z, g, note, level, date, time, sncode) VALUES ('$c_num', '$year', '$moom', '$day', '$z', '$pcpout', '$fnote', '$level', '$date', '$time', '$sncode')"; 
        mysql_query($insertCommand15,$sc);
		mysql_select_db($database_sc, $sc);
    $query_Recs = sprintf("SELECT * FROM gold_sum WHERE number = '$c_num' && year=$year && moom=$moom && day=$day");
    $Recs = mysql_query($query_Recs, $sc) or die(mysql_error());
    $row_Recs = mysql_fetch_assoc($Recs);
    $totalRows_Recs = mysql_num_rows($Recs);
    if ($totalRows_Recs == 0) {
        mysql_select_db($database_sc, $sc);
        $insertCommand15="INSERT INTO gold_sum (number, year, moom, day, z, level2) VALUES ('$c_num', '$year', '$moom', '$day', '$z', '$gold_f')"; 
        mysql_query($insertCommand15,$sc);
        } else {
	        $new_level2=$row_Recs['level2']+$gold_f;
	        $update11="UPDATE gold_sum SET level2= $new_level2 WHERE number = '$c_num' && year=$year && moom=$moom && day=$day";
            mysql_select_db($database_sc, $sc);
            $Result11 = mysql_query($update11, $sc) or die(mysql_error());
	        }
		/*
    ...
    mysql_query($insertCommand15,$sc);*/
		//
		$atx=1;
		$update11="UPDATE fd SET at=$atx,fnumber='$fdcard',okdata='$date' WHERE id=$tid";
        mysql_select_db($database_sc, $sc);
        $Result11 = mysql_query($update11, $sc) or die(mysql_error());
		//
