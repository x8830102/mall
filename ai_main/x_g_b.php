...
switch ($pudid) {case'1':$tt_us=$row_Reccd2['m_guser2'];$tt_a="m_guser2";$tt_a2="gtow2";$tt_b=" && prd2=2";$tt_b2="prd2=3";$tt_b3=" && prd2=3";break;
                 case'2':$tt_us=$row_Reccd2['m_guser2'];$tt_a="m_guser2";$tt_a2="gtow2";$tt_b=" && prd2=2";$tt_b2="prd2=3";$tt_b3=" && prd2=3";break;
				 case'3':$tt_us=$row_Reccd2['m_guser3'];$tt_a="m_guser3";$tt_a2="gtow3";$tt_b=" && prd3=2";$tt_b2="prd3=3";$tt_b3=" && prd3=3";break;}
//7p-yes
$p_total=0;
mysql_select_db($database_kg, $kg);
...
if ($p_total == 6) {//升3
	...
	if ($totalRows_Recx < 2) {$xgu="SN160419083";
		...
		while ($xa ==0) {
			mysql_select_db($database_kg, $kg);
            $query_Recx2 = sprintf("SELECT * FROM memberdata WHERE m_guser3 = '$x2' ORDER BY gtow2");
	...
	//
	mysql_select_db($database_kg, $kg);
        $query_Recgg = sprintf("SELECT * FROM memberdata WHERE m_guser3='$xgu'");
	...
	$prd3=3;
	$update11="UPDATE memberdata SET m_guser3='$xgu', gtow3='$gw', prd3=$prd3 WHERE number='$tt_us'";
    mysql_select_db($database_kg, $kg);
	...
	while ($bw == 0) {
		...
		if ($totalRows_Recgg < 2) {
			...
			*/
			...
			$mgold=$row_Recgg2['pay2'];
			$mnote="第二局结束后，奖金 = ".$mgold;
			mysql_select_db($database_kg, $kg);
	...
	$mnotex="第二局结束后奖金,扣「商城管理费10%」,扣 「税10%」,扣 「贵金属托管费5%」,扣 「平台系统维护费 15美元」";
	mysql_select_db($database_kg, $kg);
...
//升3--?>
<script type="text/javascript">document.location.href="x_g_c.php?tt_q=<?php echo $tt_us;?>";</script>
    <?php
    } else {
