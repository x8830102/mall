...
<div class="prd-block prd-block3" id="<?php echo $pdid; ?>">
	...
	<div class="btn-wrp<?php echo $btnwrp_cls; ?>">
		...
		<?php 
		if ($stock_status && isset($product['quantity'])) { ?>
		...
		<a class="vm btn btn-cart<?php echo $cart_cls; ?>"  title="<?php echo $button_cart; ?>" onclick="cart.add('<?php echo $product['product_id']; ?>', '<?php echo isset($product['minimum']) ? $product['minimum'] : 1; ?>','1');" >立即購買</a>
		<a class="btn btn-cart<?php echo $cart_cls; ?>" title="加入願望清單" onclick="wishlist.add('<?php echo $product['product_id']; ?>');">加入願望清單</a>
		<a  class="btn btn-cart<?php echo $cart_cls; ?>" title="加入比較" onclick="compare.add('<?php echo $product['product_id']; ?>');">加入比較清單</a>
		<?php } else { ?>
