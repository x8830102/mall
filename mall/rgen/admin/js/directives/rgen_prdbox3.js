(function(){
	...
	angular.module('rgenDirectives').directive('rgenPrdbox3',['$modal', function($modal) {
		return {
			restrict: 'E',
			require: '?ngModel',
			replace: true,
			...
			templateUrl: function(el,attrs){
				...
				tpl('rgen_prdbox3_alone', 'dir'):
				tpl('rgen_prdbox3', 'dir');
			},
			scope: {
				ngModel: '=',
				label: '@',
			...
			link: function(scope, element, attrs, ctrl) {
				...
				scope.open = function(newscope) {
					var modalInstance = $modal.open({
						templateUrl: tpl('rgen_prdbox3_modal', 'dir'),
						controller: "rgenPrdbox3Modal",
						size: 'lg',
						backdrop: 'static',
						resolve: {
							ngModel: function () {
	...
	angular.module('rgenDirectives').controller('rgenPrdbox3Modal', [
		'$scope', 
		'$modalInstance', 
		'Rest', 
		'Pop',
