(function(){
	'use strict';
	angular.module('rgen').factory('customhtmlFactory', [function () {
		return {
			common: function () {
				return {
					hoverstyle: "1",
	...
	angular.module('rgen').controller('moduleCustomhtml', [
		'$scope', 
		'$state',
		function($scope, $state){
			//$scope.goToState = function(name){ $state.go(name) };
			if ($state.current.name == 'mod_customhtml') {
				$state.go('mod_customhtml.manage');
			};
	}]);

	angular.module('rgen').controller('moduleCustomhtmlData', [
		'$scope', 
		'Rest', 
		'Pop', 
		'Loader',
		...
		'Mod',
		'SweetAlert',
		function($scope, Rest, Pop, Loader, $state, $compile, $timeout, $modal, customhtmlFactory, Mod, SweetAlert){
			...
			// Common settings
			$scope.modConetnt.common = new customhtmlFactory.common();
