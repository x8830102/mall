(function(){
	'use strict';
	angular.module('rgen').factory('bannergridsFactory', [function () {
		return {
			common: function () {
				return {
					title_status: false,
			...
			block: function () {
				return {
					...
					setting: {
						...
						// Image
						type: 'normal', // 'slideshow or normal'
						bnr_image: { 
	...
	angular.module('rgen').controller('moduleBannergrids', [
		'$scope', 
		'$state',
		function($scope, $state){
			//$scope.goToState = function(name){ $state.go(name) };
			if ($state.current.name == 'mod_bannergrids') {
				$state.go('mod_bannergrids.manage');
			};
	}]);

	angular.module('rgen').controller('moduleBannergridsData', [
		'$scope', 
		'Rest', 
		'Pop', 
		'Loader',
		...
		'Mod',
		'SweetAlert',
		function($scope, Rest, Pop, Loader, $state, $compile, $timeout, $modal, bannergridsFactory, Mod, SweetAlert){
			...
			------------------------*/
			$scope.tpl = {
				common : tpl('modules/bannergrids/common'),
				manage : tpl('modules/bannergrids/manage_form')
			}
			$scope.manage_tabs = [
			...
			$scope.newSubItem = function(scope, type) {
				if (scope.depth() < 11) {
					...
					if (type == 'col') {
						var title = 'Column';
						var item_data = new bannergridsFactory.col();
					}
					if (type == 'block') {
						var title = 'Block';
						var item_data = new bannergridsFactory.block();
					}
					if (type == 'copy') {
			...
			$scope.newItem = function() {
				if ($scope.modConetnt.data.length <= 20) {
					var temp 			= new bannergridsFactory.tree_node();
						//temp.node_type 	= "main";
						temp.item_data 	= new bannergridsFactory.row();
			...
			// Common settings
			$scope.modConetnt.common = new bannergridsFactory.common();
			...
			$scope.popup = function (tplfile, item, depth) {
				var modalInstance = $modal.open({
					templateUrl: tplfile,
					controller: "moduleBannergridsModal",
					backdrop: 'static',
					size: 'lg',
					resolve: {
			...
			$scope.open = function(node, type, main) {
				...
				var tplfile = tpl('modules/bannergrids/'+type+'_modal');
