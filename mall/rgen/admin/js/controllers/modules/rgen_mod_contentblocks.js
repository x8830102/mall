(function(){
	'use strict';
	angular.module('rgen').factory('contentblocksFactory', [function () {
		return {
			common: function () {
				return {
					title_status: false,
	...
	angular.module('rgen').controller('moduleContentblocks', [
		'$scope', 
		'$state',
		function($scope, $state){
			//$scope.goToState = function(name){ $state.go(name) };
			if ($state.current.name == 'mod_contentblocks') {
				$state.go('mod_contentblocks.manage');
			};
	}]);

	angular.module('rgen').controller('moduleContentblocksData', [
		'$scope', 
		'Rest', 
		'Pop', 
		'Loader',
		...
		'Mod',
		'SweetAlert',
		function($scope, Rest, Pop, Loader, $state, $compile, $timeout, $modal, contentblocksFactory, Mod, SweetAlert){

			$('body').removeClass('single-level');

			var modKey = 'contentblocks';
			...
			$scope.newSubItem = function(scope, type) {
				if (scope.depth() < 11) {
					...
					if (type == 'col') {
						var title = 'Column';
						var item_data = new contentblocksFactory.col();
					}
					if (type == 'block') {
						var title = 'Block';
						var item_data = new contentblocksFactory.block();
					}
					if (type == 'copy') {
			...
			$scope.newItem = function() {
				if ($scope.modConetnt.data.length <= 20) {
					var temp 			= new contentblocksFactory.tree_node();
						//temp.node_type 	= "main";
						temp.item_data 	= new contentblocksFactory.row();
			...
			// Common settings
			$scope.modConetnt.common = new contentblocksFactory.common();
			...
			$scope.popup = function (tplfile, item, depth) {
				var modalInstance = $modal.open({
					templateUrl: tplfile,
					controller: "moduleContentblocksModal",
					backdrop: 'static',
					size: 'lg',
					resolve: {
			...
			$scope.open = function(node, type, main) {
				...
				var tplfile = tpl('modules/contentblocks/'+type+'_modal');
