(function(){
	'use strict';
	angular.module('rgen').factory('gridmanagerFactory', [function () {
		return {
			common: function () {
				return {
					title_status: false,
	...
	angular.module('rgen').controller('moduleGridmanager', [
		'$scope', 
		'$state',
		function($scope, $state){
			//$scope.goToState = function(name){ $state.go(name) };
			if ($state.current.name == 'mod_gridmanager') {
				$state.go('mod_gridmanager.assign');
			};
	}]);

	angular.module('rgen').controller('moduleGridmanagerData', [
		'$scope', 
		'Rest', 
		'Pop', 
		'Loader',
		...
		'Mod',
		'SweetAlert',
		function($scope, Rest, Pop, Loader, $state, $compile, $timeout, $modal, gridmanagerFactory, Mod, SweetAlert){

			$('body').removeClass('single-level');

			var modKey = 'gridmanager';
			...
			------------------------*/
			$scope.tpl = {
				common : tpl('modules/gridmanager/common'),
				manage : tpl('modules/gridmanager/manage_form')
			}
			$scope.manage_tabs = [
			...
			$scope.newSubItem = function(scope, type) {
				if (scope.depth() < 11) {
					...
					if (type == 'col') {
						var title = 'Column';
						var item_data = new gridmanagerFactory.col();
					}
					if (type == 'block') {
						var title = 'Block';
						var item_data = new gridmanagerFactory.block();
					}
					if (type == 'copy') {
			...
			$scope.newItem = function() {
				if ($scope.modConetnt.data.length <= 20) {
					var temp 			= new gridmanagerFactory.tree_node();
						//temp.node_type 	= "main";
						temp.item_data 	= new gridmanagerFactory.row();
			...
			// Common settings
			$scope.modConetnt.common = new gridmanagerFactory.common();
			...
			$scope.popup = function (tplfile, item, depth) {
				var modalInstance = $modal.open({
					templateUrl: tplfile,
					controller: "moduleGridmanagerModal",
					backdrop: 'static',
					size: 'lg',
					resolve: {
			...
			$scope.open = function(node, type, main) {
				...
				var tplfile = tpl('modules/gridmanager/'+type+'_modal');
