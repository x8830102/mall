(function(){
	'use strict';
	angular.module('rgen').factory('contentsliderFactory', [function () {
		return {
			common: function () {
				return {
					title_status: false,
	...
	angular.module('rgen').controller('moduleContentslider', [
		'$scope', 
		'$state',
		function($scope, $state){
			//$scope.goToState = function(name){ $state.go(name) };
			if ($state.current.name == 'mod_contentslider') {
				$state.go('mod_contentslider.manage');
			};
	}]);

	angular.module('rgen').controller('moduleContentsliderData', [
		'$scope', 
		'Rest', 
		'Pop', 
		'Loader',
		...
		'Mod',
		'SweetAlert',
		function($scope, Rest, Pop, Loader, $state, $compile, $timeout, $modal, contentsliderFactory, Mod, SweetAlert){

			$('body').removeClass('single-level');

			var modKey = 'contentslider';
			...
			$scope.modId = null;
			$scope.modStorage = {};
			$scope.namePrefix = 'Content slider';
			...
			------------------------*/
			$scope.tpl = {
				common : tpl('modules/contentslider/common'),
				manage : tpl('modules/contentslider/manage_form')
			}
			$scope.manage_tabs = [
			...
			$scope.newSubItem = function(scope, type) {
				if (scope.depth() < 11) {
					...
					if (type == 'col') {
						var title = 'Column';
						var item_data = new contentsliderFactory.col();
					}
					if (type == 'block') {
						var title = 'Block';
						var item_data = new contentsliderFactory.block();
					}
					if (type == 'copy') {
			...
			$scope.newItem = function() {
				if ($scope.modConetnt.data.length <= 20) {
					var temp 			= new contentsliderFactory.tree_node();
						//temp.node_type 	= "main";
						temp.item_data 	= new contentsliderFactory.row();
			...
			// Common settings
			$scope.modConetnt.common = new contentsliderFactory.common();
			...
			$scope.popup = function (tplfile, item, depth) {
				var modalInstance = $modal.open({
					templateUrl: tplfile,
					controller: "moduleContentsliderModal",
					backdrop: 'static',
					size: 'lg',
					resolve: {
			...
			$scope.open = function(node, type, main) {
				...
				var tplfile = tpl('modules/contentslider/'+type+'_modal');
