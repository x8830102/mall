		...
		if (isset($data['totals'])) {
			...
			if($total['code'] == 'coupon') {
				...
				if($total['code'] == 'reward'){
					...
					$query = $pdo_oc->query("SELECT number FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$data['customer_id'] . "' AND status = '1'");
					$number = $query->fetch();
					$query = $pdo_cmg->query("INSERT INTO r_cash SET number = '" . $number['number'] . "', 
							note = '於購物商城消費" . (float)-$total['value'] . "點<br>訂單編號" . (int)$order_id . "',
							note2 = '". (int)$order_id . "',
							cout = '" . (float)-$total['value'] . "', 
							csum = ((SELECT csum FROM (SELECT csum FROM r_cash WHERE number = '" . $number['number'] . "' ORDER BY id DESC limit 1) AS csum)-". (float)-$total['value'] ."), 
							date = CURDATE(), 
							time = CURTIME()");
					$pdo_oc = NULL;
					$pdo_cmg = NULL;
