<?php
$user_name=$_POST['n_name'];$user_email=$_POST['n_email'];$fn=$_POST["fn"];//echo $user_name;
...
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( !is_subdomain_install() )
		...
		echo '<input name="blogname" style="display:none;"type="text" id="blogname" value="'.esc_attr($blogname).'" maxlength="60" /><span class="suffix_address"></span><br />';
	...
	// Blog Title
	?>
	<label  style="color: #FF0000">&#9733;必填：</label>
	<label ><?php _e('網站商店名稱設定：') ?><!--<a href="#notice1" class="button" style="position:relative; font-family:新細明體; font-size:13px; color:#fff; left:10px;">示意圖</a>-->
	<?php if ( $errmsg = $errors->get_error_message('blog_title') ) { ?>
		<p class="error"><?php echo $errmsg ?></p></label>
	<?php }
	echo '<input name="blog_title" type="text" id="blog_title" value="'.esc_attr($blog_title).'" />';
	?>
	<label><?php _e('可自由發揮創意，啟用後可進入個人網站後台修改') ?></label>
 ...
 * @param WP_Error|string $errors     A WP_Error object containing existing errors. Defaults to empty string.
 */
//echo '<label for="recommend"><span style="color: #DDDDDD">' . __('分享來源： ')?><? //echo $fn;?><? '</span></label>';
...
function show_user_form($user_name = '', $user_email = '', $errors = '') {
	...
	/*$_POST['n_name'] = "iremei";
	$_POST['n_email'] = "iremei@xx.xx";*/
	// User name
	echo '<label for="user_name">您的登入帳號 : '.$_POST['n_name'].'</label>';
	if ( $errmsg = $errors->get_error_message('user_name') ) {
		echo '<p class="error">'.$errmsg.'</p>';
	}
	echo '<input name="user_name" type="hidden" id="user_name" value="'. esc_attr( $_POST['n_name'] ) .'"<br />';
	//_e( '(Must be at least 4 characters, letters and numbers only.)' );
	?>

	<label for="user_domain"><?php _e( '您的網址名稱 : ' ); echo ($_POST['n_name']);?>.lifelink.com.tw</label>

	<label for="user_email"><?php _e( '您的郵件地址 : ' ); echo ($_POST['n_email']);?></label>
	<?php if ( $errmsg = $errors->get_error_message('user_email') ) { ?>
		<p class="error"><?php echo $errmsg ?></p>
	<?php } ?>
	
	<input name="user_email" type="hidden" id="user_email" value="<?php  echo $_POST['n_email'] ?>" maxlength="200" /><?php //_e('We send your registration email to this address. (Double-check your email address before continuing.)') ?>
	<?php
...
function validate_user_form() {
	return wpmu_validate_user_signup($_POST['user_name'], $_POST["user_email"],"BtoC");
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	<form id="setupform" method="post" action="BtoC-signup.php">
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	...
	?></h2>
	<form id="setupform" method="post" action="BtoC-signup.php" novalidate="novalidate">
		<br>
		<span style="color: #d6d6d6"><? echo "分享來源： ",$_POST["fn"];?></span>
        <input name="fn" type="hidden" id="fn" value="<? echo $_POST["fn"];?>" />
	  ...
	  <p class="submit" style="margin-bottom: 100px"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e('Next') ?>" /></p>
	</form>
	<?php
}
...
function validate_blog_signup() {
	...
	$user_result = wpmu_validate_user_signup( $_POST['user_name'], $_POST["user_email"],"BtoC" );
	...
	$fn=$_POST['fn'];//echo "%%%",$fn;exit;
	mysql_select_db($database_sr, $sr);
	$update11="UPDATE wp_signups SET fname='$fn' WHERE user_login='$user_name'";

    $Result11 = mysql_query($update11, $sr) or die(mysql_error());
	return true;
}
...
function confirm_blog_signup( $domain, $path, $blog_title, $user_name = '', $user_email = '', $meta = array() ) {
	...
	$query_Reclat = sprintf("SELECT * FROM wp_signups WHERE user_login='$user_name' ");
	$Reclat = mysql_query($query_Reclat, $tw) or die(mysql_error());
	$row_Reclat = mysql_fetch_assoc($Reclat);
	$a_key = $row_Reclat["activation_key"];?>
	<h2>
	<?php /* translators: %s: site address */
	printf( __( '<div style="text-align:center;">恭喜您! <p>您的新網站 <p>【%s】 <p>已經準備好了' ), "<span style='color:#D13030;'>{$blog_title}</span>" );
	$fn = $_POST['fn'];
	?></h2>
	<HR><p>
	<script>
	...
	<?php echo "您位於串門子的網址是:<span>http://{$domain}</span><br></p><p style=' font-size:22px;'>請點此連結啟用網站:<br><button id='create' style='color:#D13030;background:#fff;border: 0px;font-size: 24px;margin-left: -19px;' onclick='create()' >http://{$domain}/wp-activate.php?key=$a_key</button>";?></p>
	<span id="w"style="display:none">網站建置中..請稍後!</span>
	
	<form id="fn_form" action="http://<?php echo $domain;?>/wp-activate.php?key=<?php echo $a_key;?>" method="post">
		<input type="hidden" name="fn" id="fn" value="<?php echo $fn;?>"  />
		<input type="hidden" name="btc" id="btc" value="btc"  />
	</form>
	<p><?php /* translators: %s: email address 
	printf( __( 'Check your inbox at %s and click the link given.' ), '<strong>' . $user_email . '</strong>' ); */?>	
	<!--<span style="color:#D13030; font-size:22px;">若未收到e-mail請至垃圾郵件尋找</span>，有時會誤送進去。-->
	<p><!--
		<?php// _e( 'If you haven&#8217;t received your email yet, there are a number of things you can do:' ) ?>-->
		<?php
			/* translators: %s: email address */
			/*printf( __( '請確認您輸入的e-mail : <span style="color:#D13030;"> %s</span>是否正確，若不正確您將無法收到您的e-mail，請您重新申請。' ),$user_email* );*/
		?>
	</p>
	<p><?php //_e( '若是您30分鐘內未至信箱啟用您的網站，請您必須再註冊一次。' );echo $result['$domain']; ?></p><p>
	<?php
	/** This action is documented in wp-signup.php */
	do_action( 'signup_finished' );
