...
{
    private $payment = 'suntech_sunship';
    ...
    {
        $payment = explode('_', $this->payment);
        $payment_type = 'suntech_' . $payment[1];
        try {
            $this->language->load('payment/' . $payment_type);
            $this->load->model('checkout/order');
            $this->load->model('payment/' . $payment_type);
            $data = $this->request->post;
            $pwd = $this->config->get('suntech_webatm_password');
            ...
            if ($response['shipment']) {   //物流回傳
                ...
                if ($response['paid']) { //繳費回傳
                    if ($data['errcode'] != '00') {
                        ...
                        $this->model_checkout_order->addOrderHistory($order_id, 10, $message, false);
                    } else {
                        $message = sprintf($this->language->get('text_success_response'), $data['buysafeno']);
                        if ($order_info['order_status_id'] == $this->order_init_status) {
                            $this->model_checkout_order->addOrderHistory($order_id, 5, $message, true);
                        }
                    }
                    echo '0000';
                    exit;
                    ...
                    $message = sprintf($this->language->get('text_success_notify') . '', $data['CargoNo'], $data['CargoNo'], $data['buysafeno']);
                    $shipping_info = array(
                    ...
                    $data['text_response'] = sprintf($this->language->get('text_response'), $data['CargoNo']);
