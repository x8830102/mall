...
{
    private $payment = 'suntech_paycode';
    private $init_order_status = 1;
    ...
    public function saveOrder()
    {
        ...
        $order_init_status = $this->init_order_status; // pending
        ...
        // Add Order History
        $this->model_checkout_order->addOrderHistory($order_id, $order_init_status, $this->language->get('text_order_checkout') . ', ' . $this->language->get('text_title'), true);
        // 清空購物車
            ...
            if ($response['shipment']) {   //物流回傳
                ...
                if ($response['paid']) { //繳費回傳
                    if ($data['errcode'] != '00') {
                        ...
                        if ($order_info['order_status_id'] == $this->init_order_status) {
                    ...
                    $message = sprintf($this->language->get('text_callback_notify'), $NewDate, $data['buysafeno'], $_POST['paycode']);
