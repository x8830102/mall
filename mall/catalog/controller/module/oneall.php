...
class ControllerModuleOneall extends Controller
{
	...
	public function register()
	{
		...
		// Telephone Number user_phone_number
		...
		{
			// Restore from social network profile
			if ( ! empty ($user_data ['user_phone_number']))
			{
				$data['telephone'] = $user_data ['user_phone_number'];
			}
			else
			{
				$data['telephone'] = '';
	...
	private function validate(){
		...
		if ($this->request->post['username']) {
			$stmt = $pdo_cmg->query("SELECT m_username FROM memberdata WHERE m_username = '".$this->request->post['username']."'");
			$check = $stmt->fetch();
			if($check){
		...
		if ($this->request->post['fname']) {
			$stmt = $pdo_cmg->query("SELECT m_username FROM memberdata WHERE m_username = '".$this->request->post['fname']."'");
			$check = $stmt->fetch();
			$pdo_cmg = NULL;
			if(empty($check)){
