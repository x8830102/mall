...
class ModelPaymentSuntech24Pay extends Model
{
    public function getMethod($address, $total)
    {
        $this->load->language('payment/suntech_24pay');

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('suntech_24pay_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if (!$this->config->get('suntech_24pay_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                ...
                'terms' => '',
                'sort_order' => $this->config->get('suntech_24pay_sort_order')
            );
        }

        if ($this->config->get('suntech_24pay_cargo_option')) {
