...
class ModelPaymentSuntechBuysafe extends Model
{
    public function getMethod($address, $total)
    {
        $this->load->language('payment/suntech_buysafe');

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('suntech_buysafe_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

        if (!$this->config->get('suntech_buysafe_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                'code' => 'suntech_buysafe',
                'title' => $this->language->get('text_title'),
                'terms' => '',
                'sort_order' => $this->config->get('suntech_buysafe_sort_order')
            );
        }

        if (count($this->config->get('suntech_buysafe_installments')) || $this->config->get('suntech_buysafe_cargo_option')) {
            $method_data['title'] .= "<p style='display: inline;color: tomato;'>（下一步可選擇";
            if (count($this->config->get('suntech_buysafe_installments'))) {
                $method_data['title'] .= '<b>分期期數</b>';
                if ($this->config->get('suntech_buysafe_cargo_option')) {
                    $method_data['title'] .= '及是否<b>超商取貨</b>';
                }
            } else {
                $method_data['title'] .= '是否<b>超商取貨</b>';
            }
            $method_data['title'] .= ')</p>';
        }
