...
class ModelCatalogVendorLogo extends Model {
	...
	Public function getvendorproducts($product_id) {
		$query = $this->db->query("SELECT vds.vendor_id, vds.vendor_image, vds.vendor_name FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "vendor vd ON (pd.product_id = vd.vproduct_id) LEFT JOIN " . DB_PREFIX . "vendors vds ON (vd.vendor = vds.vendor_id) WHERE p.product_id = '" . $product_id . "'");
		return $query->row;
	}
	
	public function getVendorRating($vendor_id) {
		$query = $this->db->query("SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review WHERE status = '1' AND vendor_id = '" . (int)$this->db->escape(isset($vendor_id) ? $vendor_id : 0) . "' GROUP BY vendor_id");
	...
	public function getVendorAverageRating($product_id) {
		...
		$query = $this->db->query("SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review WHERE status = '1' AND vendor_id = '" . (int)$this->db->escape(isset($vendor->row['vendor_id']) ? $vendor->row['vendor_id'] : 0) . "' GROUP BY vendor_id");
	...
	public function getTheVendors($data) {
		$sql = "SELECT *, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review WHERE vendor_id = vds.vendor_id AND status = '1' GROUP BY vendor_id) AS rating, vds.firstname as firstname, vds.lastname as lastname, vds.vendor_description as description FROM " . DB_PREFIX . "vendors vds LEFT JOIN " . DB_PREFIX . "user u ON (vds.user_id = u.user_id) WHERE u.status = '1' AND vds.vendor_id IN (" . "'" . implode("','", $data['vendors']) . "'" . ")";
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'vendor_name' || $data['sort'] == 'company') {
		...
		if (isset($data['order']) && ($data['order'] == 'DESC')) {
			$sql .= " DESC, LCASE(vendor_name) DESC";
		} else {
			$sql .= " ASC, LCASE(vendor_name) ASC";
		}
	...
	public function getProducts($data = array()) {
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) 
				  ...
				  LEFT JOIN " . DB_PREFIX . "StuProduct sp ON (p.product_id = sp.product_id)
		...
		$sql .= " AND v.vendor = '" . $data['vendor_id'] . "' OR sp.student_id = '" . $data['vendor_id'] . "'";
