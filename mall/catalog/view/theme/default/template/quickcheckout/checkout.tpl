		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				<?php if ($highlight_error) { ?>
					if (json['error']['password']) {
						$('#payment-address input[name=\'password\']').css('border', '1px solid #f00');
					}

					if (json['error']['confirm']) {
						$('#payment-address input[name=\'confirm\']').css('border', '1px solid #f00');
					}
				<?php } ?>
			} else {
				<?php if ($shipping_required) { ?>
	...
	$.ajax({
		...
		success: function(html) {
			...
			<?php } else { ?>
				...
				$('html, body').animate({ scrollTop: $('#onepagecheckoutcart').offset().top }, 'slow');
...
function reloadVoucher() {
	$.ajax({
		...
		beforeSend: function() {
			...
			moduleLoad($('#voucher'), <?php echo $loading_display; ?>);
		},
		success: function(html) {
			moduleLoaded($('#voucher'), <?php echo $loading_display; ?>);
			alert(html);
			
			$('#voucher .onepage-ch-content').html(html);
		},
		<?php if ($debug) { ?>
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
