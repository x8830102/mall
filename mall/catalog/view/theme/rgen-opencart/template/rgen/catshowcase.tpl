					...
					<?php foreach ($modules['data'] as $row) { ?>
						<div class="rw <?php echo $row['row']['gutter'].' '.$row['row']['margin_b']; ?>">
							...
							<div class="cl <?php echo $column['col_size']; ?><?php echo isset($column['content_view']['cssclass']) && $column['content_view']['cssclass'] != '' ? ' '.$column['content_view']['cssclass']:null; ?>">
								...
								/* Grid view
								------------------------*/
								if ($column['content_view']['view'] == 'grid' && $column['content_view']['style'] != 3) { 
									...
									<div class="catshowcase-grid rw<?php echo $eq_grids; ?>" id="catshowcase-grid-<?php echo $i; ?>">
										<?php if (isset($column['content_items']) && sizeof($column['content_items']) > 0) { ?>
										<?php foreach ($column['content_items'] as $block) { ?>
										<?php if (isset($block['category_data'])) { ?>
										<div class="cl">
											<?php include $this->rgen('catshowcase_block'.$column['content_view']['style']); ?>
										</div>
										<?php } ?>
										<?php } } ?>
									</div>
									...
									jQuery(document).ready(function($) {
										equalH('#<?php echo $modid; ?> #catshowcase-grid-<?php echo $i; ?>', '#<?php echo $modid; ?> #catshowcase-grid-<?php echo $i; ?> > .cl');
									});
									</script>
									<?php } ?>
								<?php } ?>
								...
								/* Scroll view
								------------------------*/
								if ($column['content_view']['view'] == 'carousel' && $column['content_view']['style'] != 3) { ?>
								<div 
									class         ="widget-carousel ctrl-b" 
									id            ="catshowcase-carousel-<?php echo $i; ?>"
									data-stpd     ="1" 
								...
								$("#<?php echo $modid; ?> #catshowcase-carousel-<?php echo $i; ?>").css({	opacity: 0 });
								$(document).ready(function() {
									var obj = "#<?php echo $modid; ?> #catshowcase-carousel-<?php echo $i; ?>";
								...
								?><div 
									...
									id            ="catshowcase-slideshow-<?php echo $i; ?>" 
									data-items    ="1"
									data-stpd     ="0" 
								...
								<script type="text/javascript" ><!--
								$("#<?php echo $modid; ?> #catshowcase-slideshow-<?php echo $i; ?>").css({	opacity: 0 });
								$(document).ready(function() {
									var obj = "#<?php echo $modid; ?> #catshowcase-slideshow-<?php echo $i; ?>";
									var resObj = {};
									var settings = {
