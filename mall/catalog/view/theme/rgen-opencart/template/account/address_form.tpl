...
<div class="frm-wrp">
	<h2 class="frm-hd"><?php echo $text_edit_address; ?></h2>
	<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
		<fieldset>
			...
			<div class="form-group required">
	            ...
	            <div class="col-sm-10">
	              ...
	              <?php
	                  if($zone_id){
	                    ...
	                    $stmt = $pdo_oc->prepare("SELECT * FROM oc_zone_to_district WHERE zone_id = ? ORDER BY postcode");
	                    ...
	                    foreach ($district as $district){ 
	                      if ($district['name'] == $city) {?>
	                        <option value="<?php echo $district['name']; ?>" name="<?php echo $district['zone_id']; ?>" class="district" tag="<?php echo $district['postcode']; ?>" selected="selected"><?php echo $district['name']; ?></option>
	                    <?php }else{ ?>
	                        <option value="<?php echo $district['name']; ?>" name="<?php echo $district['zone_id']; ?>" class="district" tag="<?php echo $district['postcode']; ?>" ><?php echo $district['name']; ?></option>
	                  <?php } } }?>
	              </select>
	              <?php if ($error_city) { ?>
	          ...
	          <script>
	            $(document).ready(function() {
	              var zone = "<?php echo $zone_id; ?>"; //get post district
	              var vis = document.getElementsByName(zone);
	              $(vis).css('display','block'); 

	              $('#input-zone').change(function(){
	                $("#input-postcode").attr("value",""); //empty postcode
	                zone = $('#input-zone').val();

	                //proccess zone_id to district by ajax
	                $.ajax({
	                  type: "POST",
	                  dataType: "json",
	                  data:{
	                  ...
	                  success: function(data){
	                    html2 = '<option value=""><?php echo $text_select; ?></option>';
	                    for(i = 0; i < data.length; i++){
	                      html2 += '<option value="' + data[i]['name'] + '"';

	                      if (data[i]['name'] == '<?php echo $city; ?>') {
	                        html2 += ' selected="selected"';
	                      }

	                      html2 += ' name="'+data[i]['zone_id'] +'" class="district" tag="'+ data[i]['postcode'] +'">' + data[i]['name'] + '</option>';
	                    }
	              ...
	              $('#input-city').change(function(){
	                var postcode = $("#input-city option:selected").attr("tag");
	                $("#input-postcode").attr("value",postcode); 
	              });
	            });
	          </script>
			<div class="form-group required">
			...
			<div class="form-group<?php echo ($custom_field['required'] ? ' required' : ''); ?> custom-field" data-sort="<?php echo $custom_field['sort_order']; ?>">
				...
				<div class="col-sm-10">
					<div>
						...
						<div class="radio">
							<?php if (isset($address_custom_field[$custom_field['custom_field_id']]) && $custom_field_value['custom_field_value_id'] == $address_custom_field['value']) { ?>
							<label><input type="radio" name="custom_field[<?php echo $custom_field['custom_field_id']; ?>]" value="<?php echo $custom_field_value['custom_field_value_id']; ?>" checked="checked" />
								<?php echo $custom_field_value['name']; ?></label>
							<?php } else { ?>
							<label><input type="radio" name="custom_field[<?php echo $custom_field['custom_field_id']; ?>]" value="<?php echo $custom_field_value['custom_field_value_id']; ?>" />
							<?php echo $custom_field_value['name']; ?></label>
							<?php } ?>
						</div>
						<?php } ?>
			...
			<div class="form-group<?php echo ($custom_field['required'] ? ' required' : ''); ?> custom-field" data-sort="<?php echo $custom_field['sort_order']; ?>">
				<label class="col-sm-2 control-label"><?php echo $custom_field['name']; ?></label>
				<div class="col-sm-10">
					<button type="button" id="button-custom-field<?php echo $custom_field['custom_field_id']; ?>" data-loading-text="<?php echo $text_loading; ?>" class="btn-default"><i class="fa fa-upload"></i> <?php echo $button_upload; ?></button>
		...
		<div class="buttons clearfix">
			...
			<div class="pull-right">
				<input type="submit" value="<?php echo $button_continue; ?>" class="default-btn" />
			</div>
		</div>
	</form>
</div>
