...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
			...
			$check = $stmt->fetch();
			if($check != ""){
				$stmt = $pdo_cc->query("SELECT * FROM fproduct WHERE my_us = '$user' AND yu_us = '$store' AND product_id = '$product_id'");
				$check = $stmt->fetch();
				if($check == ""){
					$stmt = $pdo_cc->query("INSERT INTO fproduct SET my_us = '$user', yu_us = '$store', product_title = '$product_title', product_id = '$product_id', date = NOW()");
				  	echo '追蹤成功!';
				}else{
					echo '已經追蹤過囉!';
				}
			...
			if ($product_info) {
				...
				$data['points'] = $product_info['maxpoints'];
	...
	$data['prd_quantity'] = $product_info['quantity'];
