...
class ModelSaleVDIOrder extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT DISTINCT(o.order_id), CONCAT(o.lastname,o.firstname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, ";
	...
	public function getOrderTotals($order_id) {
		$query = $this->db->query("SELECT (SELECT os.cost FROM " . DB_PREFIX . "order_shipping os WHERE os.order_id = '" . (int)$order_id . "' AND os.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS shipping_cost, (SELECT os.tax FROM " . DB_PREFIX . "order_shipping os WHERE os.order_id = '" . (int)$order_id . "' AND os.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS shipping_tax, (SELECT vdsc.amount FROM " . DB_PREFIX . "vendor_discount vdsc WHERE vdsc.order_id = '" . (int)$order_id . "' AND vdsc.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS coupon_amount, (SELECT vdsc.tax FROM " . DB_PREFIX . "vendor_discount vdsc WHERE vdsc.order_id = '" . (int)$order_id . "' AND vdsc.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS coupon_tax,(SELECT os.weight FROM " . DB_PREFIX . "order_shipping os WHERE os.order_id = '" . (int)$order_id . "' AND os.vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1) AS shipping_weight, (SELECT ot.title FROM " . DB_PREFIX . "order_total ot WHERE ot.order_id = '" . (int)$order_id . "' AND ot.code =  'tax' ORDER BY Order_Total_ID DESC LIMIT 1) as tax_title, (SELECT vdsct.title FROM " . DB_PREFIX . "vendor_discount vdsct WHERE vdsct.order_id = '" . (int)$order_id . "' AND vdsct.vendor_id =  '" . (int)$this->user->getVP() . "' LIMIT 1) as coupon_title, (SELECT os.title FROM " . DB_PREFIX . "order_shipping os WHERE os.order_id = '" . (int)$order_id . "' AND os.vendor_id =  '" . (int)$this->user->getVP() . "' LIMIT 1) as title, SUM(quantity*tax) as tax, SUM(total) AS total FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' AND vendor_id = '" . (int)$this->user->getVP() . "' LIMIT 1");
		return $query->rows;
	}
