...
class ControllerSaleVDIContractHistory extends Controller {
	...
	public function renew() {
   	...
   	private function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/vdi_contract_history', 'token=' . $this->session->data['token'], true)
		);
		...
		$data['renew'] = $this->url->link('sale/vdi_contract_history/renew', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['contract_history'] = $this->url->link('sale/vdi_contract_history/renew', 'token=' . $this->session->data['token'] . $url, true);
		...
		$pagination->url = $this->url->link('sale/vdi_contract_history', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$data['column_left'] = $this->load->controller('common/vdi_column_left');
		$data['footer'] = $this->load->controller('common/vdi_footer');

		$this->response->setOutput($this->load->view('sale/vdi_contract_history_list', $data));
  	}	
	...
	public function paynow() {
		...
		if ($this->config->get('mvd_signup_paypal_email')) {
			$custom_id = $this->user->getId() . ':' . $this->request->get['next_due_date'] . ':' . $this->session->data['token'];
			$request = 'cmd=_xclick';		
			...
			$request .= '&cancel_return=' . HTTPS_CATALOG . 'index.php?route=account/cancelpayment';
