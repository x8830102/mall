...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && $_SESSION['decide'] != $_POST['decide'] ) {
	...
	if ($totalRows_Recmem2 != 0) {
		//
        mysql_select_db($database_sc, $sc);
        $query_Recoc2 = sprintf("SELECT * FROM o_cash WHERE number = '$myus' ORDER BY id DESC");
		...
		if ($row_Recoc2['csum'] >= $pay) {
			...
			if ($totalRows_Recmem3 != 0) {
				...
				mysql_select_db($database_sc, $sc);
                $query_Recoc3 = sprintf("SELECT * FROM o_cash WHERE number = '$yus' ORDER BY id DESC");
				...
				$my_note="註冊積分轉出<br/>帳號:".$yuser."/".$row_Recmem3['m_nick'];
			    mysql_select_db($database_sc, $sc);
                $insertCommand13="INSERT INTO o_cash (number, cout, csum, note, note2, date, time) VALUES ('$myus', '$pay', '$new_my_sum', '$my_note', '$note2', '$date', '$time')"; 
                mysql_query($insertCommand13,$sc);
				//收
				$new_y_sum=$row_Recoc3['csum']+$pay;
				$y_note="收到來自[ ".$row_Recmem2['m_nick']." ]的註冊積分";
				mysql_select_db($database_sc, $sc);
                $insertCommand13="INSERT INTO o_cash (number, cin, csum, note, note2, date, time) VALUES ('$yus', '$pay', '$new_y_sum', '$y_note', '$note2', '$date', '$time')"; 
				
                mysql_query($insertCommand13,$sc);
				$send=1;$err="完成轉出";unset($_SESSION['opay']);unset($_SESSION['oyuser']);unset($_SESSION['onote2']);
...
<div class="cut-navbar col-lg-7 col-md-7 col-sm-7 col-xs-11 col-lg-offset-2 col-sm-offset-1">
    ...
    <div class="cut-navbar-menu col-lg-12 col-xs-12">
        <ul>
            <li class="active"> <a href="integral_management_1.php"><span>註冊積分</span></a> </li>
    ...
    <div class="menu col-lg-11 col-xs-11" align="center"><?php if ($a_pud >= 2) {?>
        <div style=" border: 1px solid #b6c9d9">
            <div class="menu-text col-lg-12 col-md-12 col-xs-12">註冊積分管理</div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="2" class="ann-t">
                            <div style=" line-height: 1px;">*註冊積分只限於訂購系統使用，不可作為其他用途</div>
                        </td>
                    </tr> <?php if ($pc >= 0) {?>
                    <tr class="menu-tr">
                    ...
                    <tr class="menu-tr">
                        <td class="menu-td">轉出帳號</td>
                        <td>
                            <input name="yuser" type="text" id="yuser" placeholder="請輸入購買者帳號">
                        </td>
                    </tr>
                    <tr class="menu-tr">
                    ...
                    <tr class="menu-tr">
                        ...
                        <td>
                            ...
                            <a href="integral_management_1.php"><img width="15px" src="img/refresh.png" alt="" style="margin-left: 3px"></a>
                        </td>
                    </tr>
        ...
        <form id="form2" name="form2" method="get" action="integral_management_1.php">
        <div style="  margin-top: 10px" class="table-responsive ">
			...
			<DIV class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<?php 
					...
					echo "<a href='integral_management_1.php?pageNum_Recoc=".$previous."'>上一頁</a>";
					for($i=$pageNum2 ;$i<=$pageNum;$i++)
					{
						if($i == $page+1)
						{
							echo "[ <a href='integral_management_1.php?pageNum_Recoc=".($i-1)."'>$i</a> ]";
						}else{
							echo " <a href='integral_management_1.php?pageNum_Recoc=".($i-1)."'>$i</a>" ;
						}
					}
					echo "<a href='integral_management_1.php?pageNum_Recoc=".$next."'>下一頁</a>";
				?>
			</DIV>
<?php //
if ($_GET['sd1'] != "") {
...
$queryString_Recoc = sprintf("&totalRows_Recoc=%d%s", $totalRows_Recoc, $queryString_Recoc);?>
            <table class="integral-tb table" width="100%" style="min-width: 100%">
                <tbody>
                    ...
                    <?php $pj=$_GET['pj']+0; $oid=$totalRows_Recoc; if ($totalRows_Recoc != 0) {do {?>
                    <tr class="integral-tr">
                        <td align="left"><?php echo $oid-$_GET['pageNum_Recoc']*10; $oid--;?></td>
                        <td>
                          <ul>
