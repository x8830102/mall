...
//test
$passtoo=$_GET['passtoo'];
$fuser=$_GET['fuser'];
$pudid=$_GET['pudid'];
$notpay=$_GET['notpay'];
$newuser=strtolower(trim($_GET['newuser']));//echo $newuser;
$fuser2=$_GET['fuser2'];
$businessRatio=$_GET['businessRatio'];
...
if ($totalRows_Recm == 0) {header(sprintf("Location: new_account-1.php?err=執行業務一帳號錯誤&fuser=".$fuser."&newuser=".$newuser."&pudid=".$pudid));exit;}
$fnumber=$row_Recm['number'];
$query_Recm = sprintf("SELECT * FROM memberdata WHERE m_username = '$fuser2' && m_ok=1");
$Recm = mysql_query($query_Recm, $sc) or die(mysql_error());
$row_Recm = mysql_fetch_assoc($Recm);
$totalRows_Recm = mysql_num_rows($Recm);
if ($totalRows_Recm == 0) {header(sprintf("Location: new_account-1.php?err=執行業務二帳號錯誤&fuser=".$fuser."&newuser=".$newuser."&pudid=".$pudid));exit;}


//
/*if ($newuser && preg_match ("/^\w+$/", $newuser)) {header(sprintf("Location: new_account-1.php?err=此購買者帳號輸入格式只能是英文加數字1&fuser=".$fuser."&newuser=".$newuser."&pudid=".$pudid));exit;}*/
/*if (is_numeric($newuser) == true) {header(sprintf("Location: new_account-1.php?err=此購買者帳號輸入格式只能是英文加數字2 &fuser=".$fuser."&newuser=".$newuser."&pudid=".$pudid));exit;}*/
if ($newuser != "") {
mysql_select_db($database_sc, $sc);
...
if($pud_p != 0)
{
	if ($row_Recoc['csum'] < $pud_p) {header(sprintf("Location: new_account-1.php?err=我的註冊積分不足&fuser=".$fuser."&newuser=".$newuser."&pudid=".$pud_p));exit;}
}
//
$i = 6;$pd[0] = 0;	$sum = "";
...
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
 <link rel="icon" href="img/life_link.jpg" type="image/x-icon" />
    <link rel="stylesheet" type="text/css" href="/life_link/css/style.css">
    <link rel="stylesheet" type="text/css" href="/life_link/css/reset.css">
    <link rel="stylesheet" type="text/css" href="bootstrap/css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="jasny-bootstrap/css/jasny-bootstrap.css">
	<link rel="stylesheet" type="text/css" href="/life_link/css/dragdealer.css">
    ...
    var aa = new Dragdealer('just-a-slider',{
		animationCallback: function(x, y) {
			...
			var districtRatio = [10-Math.round(x * 10),Math.round(x * 10)];
			$('#districtRatio').val(districtRatio);
	  }
	});
	$("#lock").hide();
	...
	function aa(value){
		if(value ==8)
		{
			$('#n_recommend').prop('checked',false);
		}else{
    ...
    <div class="menu col-lg-11 col-xs-11" align="center">
        <div style=" border: 1px solid #b6c9d9">
            ...
            <table align="center" cellpadding="10" cellspacing="3" class="menu-table" style="margin: 50px auto 30px ">
                <tbody>
                    <tr class="menu-tr">
                      <td colspan="2" align="center"><span class="err" style="color: #F00"><?php echo $_GET['err'];?></span></td>
                    </tr>
                    <tr class="menu-tr">
                        ...
                        <input name="businessRatio" type="hidden" id="businessRatio" value="<?php echo $businessRatio;?>" /> <!--業務配比-->
						<input name="districtRatio" type="hidden" id="districtRatio" value="" /><!--行政區配比-->
						...
						<input name="notpay" type="hidden" id="notpay" value="<?php echo $_GET['notpay'];?>" />
                    ...
                    <tr class="menu-tr">
                        <td class="menu-td"><span>上架系統專案</span></td>
                    ...
                    <tr class="menu-tr">
                        <td class="menu-td">店家名稱設定</td>
                        <td>
                            <input name="nick" type="text" id="nick" value="<?php if ($_GET['nick'] != "") {echo $_GET['nick'];}?>" placeholder="請輸入中文名稱">
                        </td>
                    </tr>
					<tr class="menu-tr">
					
						<td  class="menu-td">講師帳號登記</td>
						<td >
                    ...
                    <tr class="menu-tr">
                        <td class="menu-td">行政分配區域</td>
                        <td>
                        <?php 
                            $pdo = new PDO("mysql:host=localhost;dbname=cmg58891_a;charset=utf8","cmg58891_a","cmg911com"); 
                            $query = $pdo->query("SELECT * FROM pay_district WHERE superior IS null ORDER BY id");
                        ...
                        <select name='district' id='district'>
                            ...
                            <optgroup label="<?php echo $query['name'];?>"> <?php
                                $query2 = $pdo->query("SELECT * FROM pay_district WHERE superior = '".$query['id']."'");
                                $query2 = $query2->fetchAll();
                                foreach($query2 as $query2){ ?>
                                    <option value="<?php echo $query2['number'];?>"><?php echo $query2['name'];?></option>
                                <?php } ?>
                            </optgroup>
                            <?php } ?>
                        </select>
					 ...
					 <tr class="percent menu-tr">
                        <td class="menu-td">積分配比</td>
						 ...
						 <td class="distric">
							<image id="unlock" src="img/unlocked.png">
							<image id="lock" src="img/padlock.png">
                    ...
                    <tr class="menu-tr">
                        ...
                        <td>
                            <input maxlength="10" name="callphone" type="tel" id="callphone" value="<?php if ($_GET['callphone'] != "") {echo $_GET['callphone'];}?>" />
                        </td>
                    </tr>
                    <tr class="menu-tr">
                      <td colspan="2" class="ann-t"><div style="margin-left: 124px;line-height: 1px;margin-top: 6px;color: #3493f9">*請確實填寫以利收件啟動系統</div></td>
                    </tr>
                    <tr class="menu-tr">
					...
					<tr class="menu-tr">
						<br>
						<td class="menu-td">審核</td>
						<td><input style="height:13px;" type="checkbox"  value="8" id="recommend" onClick="aa(this.value);"/>推薦
						<input style="height:13px;" type="checkbox"  value="7" id="n_recommend" onClick="aa(this.value);"/>不推薦</td>
					</tr>
					<?php 
					}
					?>
                    ...
                    <tr class="menu-tr">
                        <td valign="top">發票開立</td>
                        <td><input name="as_at" type="radio"  style="width:  20px" value="1" checked><span style="height: 26px;vertical-align: top;margin-right: 5px">捐贈</span>
                      <input type="radio" value="2" name="as_at" style="width: 20px"><span style="height: 26px; vertical-align: top;margin-right: 5px">二聯</span></td>
                  </tr>
                    <tr class="menu-tr">
                      <td valign="top">&nbsp;</td>
                      <td valign="top"><input type="radio" value="3" name="as_at"  style="width:  20px">
                      <span style="height: 26px;vertical-align: top;margin-right: 5px">三聯</span></td>
                    </tr>
                    <tr class="menu-tr">
                      <td valign="top">&nbsp;</td>
                      <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td width="11%">&nbsp;</td>
                          <td width="89%">統一編號
                            <input name="as_number" id="as_number" value="<?php if ($_GET['as_number'] != "") {echo $_GET['as_number'];}?>" size="13" maxlength="8" />
                            <br/>
                            公司名稱
                          <input name="as_name" type="text" id="as_name" value="<?php if ($_GET['as_name'] != "") {echo $_GET['as_name'];}?>" size="13"></td>
                        </tr>
                      </table></td>
                    </tr>
                    <tr class="menu-tr">
                      <td colspan="2" align="left">
                            <ul class="agree">
                                <li><input name="st" type="checkbox" style="width: 20px" value="1" checked/><span style="height: 26px; vertical-align: top">同意&nbsp;</span></li>
                        ...
                        <td colspan="2" align="left">
                            <ul class="agree">
                                <li style="margin-top: -10px">
                                    <input type="checkbox" name="st2" value="1" style="width: 20px"><span style="height: 26px; vertical-align: top">同意&nbsp;</span>
                                </li>
                                <li  style="margin-top: -15px;margin-right: 0px">
                                    <a id="myBtn"><span>會員使用條款協議與隱私政策</span></a> 
                                </li>
                    ...
                    <tr align="center">
                        <td colspan="2">
                            <input type="submit" name="button2" id="button2" class="menu-but" value="確認">
                            <input name="Submit4" type="button" class="menu-but" onclick="window.location='new_account-1.php'" value=" 取消 " />
                        </td>
                    </tr>
                </tbody>
            </table></form>
