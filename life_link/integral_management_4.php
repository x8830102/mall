	...
	if ($totalRows_Recmem2 != 0) {
		...
		if ($row_Recoc2['csum'] >= $pay) {
			...
			if ($totalRows_Recmem3 != 0) {
				...
				$my_note="紅利積分轉出<br/>帳號:".$yuser."/".$row_Recmem3['m_nick'];
			    mysql_select_db($database_sc, $sc);
        ...
        <div style="  margin-top: 10px" class="table-responsive ">
          ...
          <div class="inquire col-lg-12 col-md-12 col-sm-12 col-xs-12" align="left">
			...
			<DIV class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<?php 
					$pagecount = floor(($totalRows_Recocz-1)/10)+1;
					...
					echo "<a href='integral_management_4.php?pageNum_Recoc=".$previous."'>上一頁</a>";
					for($i=1 ;$i<=$pageNum;$i++)
					{
						if($i == $page+1)
						{
							echo "[ <a href='integral_management_4.php?pageNum_Recoc=".($i-1)."'>$i</a> ]";
						}else{
							echo " <a href='integral_management_4.php?pageNum_Recoc=".($i-1)."'>$i</a>" ;
						}
					}
					echo "<a href='integral_management_4.php?pageNum_Recoc=".$next."'>下一頁</a>";
				?>
			</DIV>
<?php //
if ($_GET['sd1'] != "") {
...
if(empty($_GET['page'])){
	$pageNum_Recoc = 0;
}else{$pageNum_Recoc = $_GET['page']*10;}
...
//?>
            <table class="integral-tb table" width="100%" style="min-width: 100%">
                <tbody>
                    ...
                    <?php $pj=$_GET['pj']+0;$rid=$totalRows_Recoc ;if ($totalRows_Recoc != 0) {do {?><tr class="integral-tr">
                        <td align="left"><?php echo $rid-$_GET['pageNum_Recoc']*10; $rid--;?></td>
                        <td>
                          <ul>
