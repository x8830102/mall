...
$vfdcard=$_SESSION['fdcard'];$vf=$_SESSION['vf'];//echo $vfdcard;exit;
$fdname=$_POST['fdname'];$fu=$_POST['fu'];$gu=$_POST['gu'];$w=$_POST['w'];$number=$_POST['number'];$m_passtoo=$_POST['m_passtoo'];$pudp=$_POST['pudp'];//$fdcard=$_SESSION['fdcard'];
?>
...
<table width="400" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td align="center">進度： 1 /2</td>

  </tr>

</table>
<?php //echo $vfdcard;exit;

//if ($row_Reccd['a_pud'] >= 3) {
/*
	...
	while ($wi != 0) {
        ...
        $query_Reccd2 = sprintf("SELECT * FROM fd WHERE card = '$fd_t_card'");
	...
	$tt_us=$fd_t_card;$keyok=0;//echo $tt_us;exit;
...
if ($totalRows_Recg != 0) {$p_total=$p_total+$totalRows_Recg;
	...
	mysql_select_db($database_sc, $sc);
        ...
        $query_Reccd2 = sprintf("SELECT * FROM fd WHERE card = '$tt_us'");
		...
		$c_name=$row_Reccd2['name'];

		$c_num=$row_Reccd2['number'];//echo $c_num;exit;

		$cid=$row_Reccd2['id'];

mysql_select_db($database_sc, $sc);

$query_Reccd3 = sprintf("SELECT * FROM fd WHERE number = '$c_num' && at=1");
...
$query_Reccd31 = sprintf("SELECT * FROM fd WHERE c_fuser = '$tt_us'");

$Reccd31 = mysql_query($query_Reccd31, $sc) or die(mysql_error());

$row_Reccd31 = mysql_fetch_assoc($Reccd31);

$totalRows_Reccd31 = mysql_num_rows($Reccd31);//echo $totalRows_Reccd31;exit;
...
if ($totalRows_Reccd31 > $totalRows_Reccd3) {$keyok=1;} else {
	...
	$cfu=$row_Recob2['m_fuser'];//echo $cfu;exit;
...
if ($row_Recfg2['a_pud'] >= 3) {
    ...
    $Result11 = mysql_query($update11, $sc) or die(mysql_error());
		...
		$update11="UPDATE fd SET at=1,fnumber='$fdcard',okdata='$date' WHERE id=$cid";
...
$insertCommand13="INSERT INTO fd (name, number, card, c_fuser, c_guser, gtow, year, moom, day, end_y, end_m, end_d, date, time) VALUES ('$c_name', '$c_num', '$fdcard', '$tt_us', '$xgu', '$gw', '$year', '$moom', '$day', '$nyear', '$moom', '$day', '$date', '$time')"; 
...
mysql_select_db($database_sc, $sc);
$insertCommand13="INSERT INTO pay_b (pout, psum, number, card, at, date, time, year, moom, day) VALUES ('$pcpout', '$newcpsum', '$c_num', '$fdcard', '$pat', '$date', '$time', '$year', '$moom', '$day')"; 
mysql_query($insertCommand13,$sc);
        //
		$level=2;
		$fnote="開福卡積分(".$tt_us.")";
	...
	$torg=111;
			...
			if ($p_total == 6) {
                ...
                $insertCommand15="INSERT INTO fd_take (number, name, fcard, date, time) VALUES ('$c_num', '$c_name', '$tt_us', '$date', '$time')"; 
...
//echo "@@##";exit;*/
$torg = 0;
if ($torg == 0) {?><script type="text/javascript">document.location.href="x_g_fd_b.php";</script>

<?php } else {$_SESSION['fdcard']=$fdcard;?><script type="text/javascript">document.location.href="x_g_fd.php";</script><?php }?>