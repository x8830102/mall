...
$vfdcard=$_SESSION['fdcard'];$vf=$_SESSION['vf'];$fu=$_SESSION['fu'];$gu=$_SESSION['gu'];$number=$_SESSION['nsn'];//echo $sn;exit;
?>
<head>
...
if ($totalRows_Reccd != 0) {
	do {$c_num=$row_Reccd['number'];
        ...
        $tt_us=$row_Reccd['card'];//echo $c_num;exit;
		$tid=$row_Reccd['id'];
		$position = $row_Reccd['filling_position'];
...
$a_pud=$row_Recm2['a_pud'];
echo $c_num.",".$fname.",".$tt_us.",".$position."<br>";
mysql_select_db($database_sc, $sc);
...
if ($p_total == 6) {//echo $tt_us;exit;
	...
	$cid=$row_Reccd2['id'];
	$c_name=$row_Reccd2['number'];//echo $cid;exit;
	/*
	//allsum 
	mysql_select_db($database_sc, $sc);
...
if ($p_total == 6 && $keyok == 1) {//echo "出局";exit;
	...
	if ($totalRows_Recgg == 0) {$gw="L";} else {if ($row_Recgg['gtow'] == "L") {$gw="R";} else {$gw="L";}}
		...
		mysql_select_db($database_sc, $sc);
$insertCommand13="INSERT INTO fd (name, number, card, c_fuser, c_guser, gtow, year, moom, day, end_y, end_m, end_d, date, time) VALUES ('$fname', '$c_num', '$fdcard', '$tt_us', '$xgu', '$gw', '$year', '$moom', '$day', '$nyear', '$moom', '$day', '$date', '$time')"; 
mysql_query($insertCommand13,$sc);}
        $vf=$fdcard;
		//out
mysql_select_db($database_sc, $sc);
...
//echo "fd2-ok";exit;*/
//
?><script type="text/javascript">document.location.href="/life_link/new_account-4.php?position=<?php echo $_SESSION['original'];?>";</script>
<?php exit;?>