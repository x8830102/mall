...
session_start();
if ($_SESSION['sn'] == "") {header(sprintf("Location: /life_link/index.php"));exit;} else {$sn=$_SESSION['sn'];}//echo $sn;exit;
...
$fus=$_SESSION['gfus'];$newnr=$_SESSION['newnr'];$pudid=$_SESSION['pudid'];$vfdcard=$_SESSION['fdcard'];$torg=111;
...
if ($row_Reccd['a_pud'] >= 6) {$fdsql="fd2";$pcpin=150000;$pcpout=300000;}
...
if ($totalRows_Recg != 0) {$p_total=$p_total+$totalRows_Recg;
    ...
    $Result11 = mysql_query($update11, $sc) or die(mysql_error());
		...
		//我系大公排
			...
			$select_position = "SELECT * FROM fd WHERE number ='$fus'  ORDER BY id ASC" ;
		...
		if ($totalRows_Recxf == 0) {
			...
			mysql_select_db($database_sc, $sc);
			$fd_amount = -1;
			$insertCommand13="INSERT INTO fd (number, card, name, c_fuser, c_guser, gtow, filling_position, fd_amount, year, moom, day, end_y, end_m, end_d, date, time, fnumber) VALUES ('$fus', '$fdcard', '$m_nick', '$fcard', '$g_user', '$gw', '$index','$fd_amount', '$year', '$moom', '$day', '$nyear', '$moom', '$day', '$date', '$time','問號a?')"; 
			mysql_query($insertCommand13,$sc); 
		}
/*
        //in
...
if ($torg == 0) 
{
	...
	<?php 
		...
		$_SESSION['fdcard']=$fdcard;?><script type="text/javascript">document.location.href="x_g_a.php";</script>
	<?php 
	}
	?>
