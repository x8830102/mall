...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && $_SESSION['decide'] != $_POST['decide'] ) {
    ...
    $myus=$_POST['number'];$passtoo=$_POST['passtoo'];$note2=$_POST['note2'];
	...
	if ($totalRows_Recmem2 != 0) {
		//
        mysql_select_db($database_sc, $sc);
        $query_Recoc2 = sprintf("SELECT * FROM g_cash WHERE number = '$myus' ORDER BY id DESC");
		...
		if ($row_Recoc2['csum'] >= $pay) {
			...
			if ($totalRows_Recmem3 != 0) {
				...
				if($username != $yuser)
				{
					...
					mysql_select_db($database_sc, $sc);
					$query_Recoc3 = sprintf("SELECT * FROM g_cash WHERE number = '$yus' ORDER BY id DESC");
					...
					mysql_select_db($database_sc, $sc);
					$insertCommand13="INSERT INTO g_cash (number, cout, csum, note, note2, date, time) VALUES ('$myus', '$pay', '$new_my_sum', '$my_note', '$note2', '$date', '$time')"; 
					mysql_query($insertCommand13,$sc);
					//收
					$new_y_sum=$row_Recoc3['csum']+$pay;
					$y_note="收到來源[ ".$row_Recmem2['m_nick']." ]的購物積分";
					mysql_select_db($database_sc, $sc);
					$insertCommand13="INSERT INTO g_cash (number, cin, csum, note, note2, date, time) VALUES ('$yus', '$pay', '$new_y_sum', '$y_note', '$note2', '$date', '$time')"; 
					mysql_query($insertCommand13,$sc);
					$send=1;$err="完成轉出";
				}else{
					$err="不能轉換給自己 !";$send=0;
...
//
mysql_select_db($database_sc, $sc);
$query_Recocz = sprintf("SELECT * FROM g_cash WHERE number = '$sn' ORDER BY id DESC");
...
<div class="cut-navbar col-lg-7 col-md-7 col-sm-7 col-xs-11 col-lg-offset-2 col-sm-offset-1">
    ...
    <div class="cut-navbar-menu col-lg-12 col-xs-12">
        <ul>
            ...
            <li class="active"> <a href="integral_management_3.php"><span>消費積分</span></a> </li>
    ...
    <div class="menu col-lg-11 col-xs-11" align="center">
        <div style=" border: 1px solid #b6c9d9">
            <div class="menu-text col-lg-12 col-md-12 col-xs-12">消費積分管理</div>
            ...
            <table align="center" cellpadding="10" cellspacing="3" class="menu-table" style="margin: 50px auto 30px ">
                <tbody>
                    ...
                    <tr class="menu-tr">
                        ...
                        <td>
                            <input name="passtoo" type="password" id="passtoo" size="12" placeholder="請輸入二級密碼"><input name="see" type="hidden" id="see" value="<?php echo $sum;?>" />
                    ...
                    <tr class="menu-tr">
                        ...
                        <td>
                            ...
                            <a href="integral_management_3.php"><img width="15px" src="img/refresh.png" alt="" style="margin-left: 3px"></a>
                        </td>
                    </tr>
        ...
        <form id="form2" name="form2" method="get" action="integral_management_3.php">
          ...
          <div class="inquire col-lg-12 col-md-12 col-sm-12 col-xs-12" align="left">
			...
			<DIV class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<?php 
					...
					echo "<a href='integral_management_3.php?pageNum_Recoc=".$previous."'>上一頁</a>";
					for($i=$pageNum2 ;$i<=$pageNum;$i++)
					{
						if($i == $page+1)
						{
							echo "[ <a href='integral_management_3.php?pageNum_Recoc=".($i-1)."'>$i</a> ]";
						}else{
							echo " <a href='integral_management_3.php?pageNum_Recoc=".($i-1)."'>$i</a>" ;
						}
					}
					echo "<a href='integral_management_3.php?pageNum_Recoc=".$next."'>下一頁</a>";
				?>
			</DIV>
<?php //
if ($_GET['sd1'] != "") {
	$sd1=$_GET['sd1'];$sd2=$_GET['sd2'];
	$key="SELECT * FROM g_cash WHERE number = '$sn' && date >= '$sd1' && date <= '$sd2' ORDER BY id DESC";
	} else {$key="SELECT * FROM g_cash WHERE number = '$sn' ORDER BY id DESC";}
//
...
$queryString_Recoc = sprintf("&totalRows_Recoc=%d%s", $totalRows_Recoc, $queryString_Recoc);?>
            <table  class="integral-tb table" width="100%" style="min-width: 100%">
                <tbody>
                    ...
                    <?php $pj=$_GET['pj']+0; $gid=$totalRows_Recoc ;if ($totalRows_Recoc != 0) {do {?><tr class="integral-tr">
                        <td align="left"><?php echo $gid-$_GET['pageNum_Recoc']*10; $gid--;?></td>
                        <td>
                          <ul>
