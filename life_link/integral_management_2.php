...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && $_SESSION['decide'] != $_POST['decide']) {
	
    $myus=$_POST['number'];$passtoo=$_POST['passtoo'];$note2=$_POST['note2'];$st=$_POST['st'];
	$pay=$_POST['pay'];$yuser=strtoupper(trim($_POST['yuser']));
	...
	if ($totalRows_Recmem2 != 0) {
		//
        mysql_select_db($database_sc, $sc);
        $query_Recoc2 = sprintf("SELECT * FROM c_cash WHERE number = '$myus' ORDER BY id DESC");
		...
		if ($row_Recoc2['csum'] >= $pay) {
			...
			if ($totalRows_Recmem3 != 0) {
				$_SESSION['decide']=$_POST['decide'];
					...
					if($username != $yuser)
					{
						...
						mysql_select_db($database_sc, $sc);
						$query_Recoc3 = sprintf("SELECT * FROM c_cash WHERE number = '$yus' ORDER BY id DESC");
						...
						$my_note="積分轉出<br/>帳號:".$yuser."/".$row_Recmem3['m_nick'];
						mysql_select_db($database_sc, $sc);
						$insertCommand13="INSERT INTO c_cash (number, cout, csum, note, note2, date, time) VALUES ('$myus', '$pay', '$new_my_sum', '$my_note', '$note2', '$date', '$time')"; 
						mysql_query($insertCommand13,$sc);
						//收
						$new_y_sum=$row_Recoc3['csum']+$pay;
						$y_note="收到[ ".$row_Recmem2['m_nick']." ]的串串積分";
						mysql_select_db($database_sc, $sc);
						$insertCommand13="INSERT INTO c_cash (number, cin, csum, note, note2, date, time) VALUES ('$yus', '$pay', '$new_y_sum', '$y_note', '$note2', '$date', '$time')"; 
						mysql_query($insertCommand13,$sc);
						$send=1;
					}else{
						$err="無法轉換給自己 !";$send=0;
			...
			if ($st == 1) {
				mysql_select_db($database_sc, $sc);
                $query_Recoc3 = sprintf("SELECT * FROM o_cash WHERE number = '$myus' ORDER BY id DESC");
                $Recoc3 = mysql_query($query_Recoc3, $sc) or die(mysql_error());
                $row_Recoc3 = mysql_fetch_assoc($Recoc3);
                $totalRows_Recoc3 = mysql_num_rows($Recoc3);
				//支
				...
				$y_note="收到源串串積分";
				mysql_select_db($database_sc, $sc);
                $insertCommand13="INSERT INTO o_cash (number, cin, csum, note, note2, date, time) VALUES ('$myus', '$pay', '$new_y_sum', '$y_note', '$note2', '$date', '$time')"; 
                mysql_query($insertCommand13,$sc);
				$send=1;$err="完成轉出";
				}
	        } else {$err="扣值不足  !";$send=0;}
	} else {$err="二级密码不對  !";$send=0;}
...
//
mysql_select_db($database_sc, $sc);
$query_Recocz = sprintf("SELECT * FROM c_cash WHERE number = '$sn' ORDER BY id DESC");
...
<div class="cut-navbar col-lg-7 col-md-7 col-sm-7 col-xs-11 col-lg-offset-2 col-sm-offset-1">
    ...
    <div class="cut-navbar-menu col-lg-12 col-xs-12">
        <ul>
            ...
            <li class="active"> <a href="integral_management_2.php"><span>串串積分</span></a> </li>
    ...
    <div class="menu col-lg-11 col-xs-11" align="center">
        <div style=" border: 1px solid #b6c9d9">
          <div class="menu-text col-lg-12 col-md-12 col-xs-12">串串積分管理</div>
            ...
            <table align="center" cellpadding="10" cellspacing="3" class="menu-table" style="margin: 50px auto 30px ">
                <tbody>
                    ...
                    <tr class="menu-tr">
                        <td ><span>串串積分結餘</span></td>
                        <td><span style="color: red;"><?php $pc=$row_Recocz['csum']+0;$pc2=number_format($pc, 0, '.' ,',');echo $pc2;?></span>&nbsp;&nbsp;&nbsp;<?php if ($pc <= 0) {echo "轉出功能暫停";}?></td>
                    </tr><?php if ($pc >= 0) {?>
                    <tr class="menu-tr">
                        <td class="menu-td">轉出積分</td>
                        <td>
                            <input name="pay" type="text" id="pay" placeholder="">
                        </td>
                    </tr>
                    <tr class="menu-tr">
                      <td align="center"  ><input name="st" type="radio" id="radio" value="1" checked="checked" style="margin-left: 91px;height: 15px;" /></td>
                      <td >轉到自己註冊積分</td>
                    </tr>
                    <tr class="menu-tr">
                      <td align="center"><input name="st" type="radio" id="radio2" value="2" style="margin-left: 91px;height: 15px;" /></td>
                      <td >
                      <input name="yuser" type="text" id="yuser" placeholder="轉至會員帳號" style="min-width: 0px;margin-bottom: 7px"></td>
                    </tr>
                    <tr class="menu-tr">
                        <td class="menu-td">備&nbsp;&nbsp;註</td>
                        <td>
                    ...
                    <tr class="menu-tr">
                        ...
                        <td>
                            ...
                            <?php echo $sum;?>
                            <a href="integral_management_2.php"><img width="15px" src="img/refresh.png" alt="" style="margin-left: 3px"></a>
                        </td>
                    </tr>
                    <tr  align="center">
					...
					<input type="hidden" name="decide" value="<?php echo $decide;?>">
                </tbody>
            </table></form>
        </div>
        ...
        <div style="  margin-top: 10px" class="table-responsive ">
            <div class="inquire col-lg-12 col-md-12 col-sm-12 col-xs-12" align="left">
              <form name="form2" method="get" action="integral_management_2.php">
                <table class="search">
                  <tbody>
                    <tr>
                      <td>查詢日期</td>
                    ...
                    <tr>
                      ...
                      <td><input type="submit" name="button2" id="button2" class="inquire-but hidden-xs" value="查詢"></td>
                    </tr>
                    <tr style="height: 5px">
                      <td></td>
                      <td></td>
                    ...
                    <tr>
                      <td  colspan="3"><input type="submit" name="button2" id="button2" class="inquire-but visible-xs" value="查詢" style="width: 100%"></td>
                    </tr>
                  </tbody>
                </table>
              </form>
			...
			<DIV class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<?php 
					if($totalRows_Recocz % 10 ==0)
					{
						$pagecount = floor($totalRows_Recocz/10);
					}else{
						$pagecount = floor($totalRows_Recocz/10+1);
					}
					...
					echo "<a href='integral_management_2.php?pageNum_Recoc=".$previous."'>上一頁</a>";
					for($i=$pageNum2 ;$i<=$pageNum;$i++)
					{
						if($i == $page+1)
						{
							echo "[ <a href='integral_management_2.php?pageNum_Recoc=".($i-1)."'>$i</a> ]";
						}else{
							echo " <a href='integral_management_2.php?pageNum_Recoc=".($i-1)."'>$i</a>" ;
						}
					}
					echo "<a href='integral_management_2.php?pageNum_Recoc=".$next."'>下一頁</a>";
				?>
			</DIV>
<?php 
if ($_GET['sd1'] != "") {
	$sd1=$_GET['sd1'];$sd2=$_GET['sd2'];
	$key="SELECT * FROM c_cash WHERE number = '$sn' && date >= '$sd1' && date <= '$sd2' ORDER BY id DESC";
	} else {$key="SELECT * FROM c_cash WHERE number = '$sn' ORDER BY id DESC";}
//
                ...
                <tbody>
                    <tr class="integral-frist-tr" align="center">
                      ...
                      <td style="width: 15%;border-top: 0px;">轉入</td>
                        ...
                        <td align="left" style="width: 20%;border-top: 0px">備註1.</td>
                        <td align="left" style="width: 20%;border-top: 0px">備註2.</td>
                    </tr>
                    <?php $pj=$_GET['pj']+0; $cid=$totalRows_Recoc;if ($totalRows_Recoc != 0) {do {?><tr class="integral-tr">
                        <td><?php echo $cid-$_GET['pageNum_Recoc']*10; $cid--;?></td>
                        <td>
                            <ul>
                        ...
                        <td align="right"><span><?php $pi=$row_Recoc['cin']+0;$pi2=number_format($pi, 0, '.' ,',');echo $pi2;?></span></td>
                        <td align="right"><span><?php $po=$row_Recoc['cout']+0;$po2=number_format($po, 0, '.' ,',');echo $po2;?></span></td>
                        <td align="right"><span><?php $pc=$row_Recoc['csum']+0;$pc2=number_format($pc, 0, '.' ,',');echo $pc2;?></span></td>
